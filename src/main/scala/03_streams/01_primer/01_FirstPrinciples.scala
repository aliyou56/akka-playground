...
object FirstPrinciples extends App {
  ...
  // Exercise: create a stream that takes the names of persons, then you will keep the first 2 names with length > 5 chars

  val names        = Seq("Alice", "Bob", "David", "scala", "akkaStreams", "Martin")
  ...
  // nameSource.via(longNameFlow).via(longNameFlow).to(nameSink).run()
